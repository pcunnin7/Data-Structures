 ...
 * go through the argument array at most once.  The only exception to this the java
 * Math.abs.  You can use the Math.abs function to compute the absolute value.
 */
public class MyFirstHomeworkFor300 {
	...
	public static int distanceBetweenMinAndMax (double[] list) {
		...
		int x = 0;
		while (i < list.length) {
			if (max < list[i]) {max = list[i]; y = i;}
			if (min > list[i]) {min = list[i]; x = i;}
			i += 1;
		}
		int z = x - y;
		return Math.abs(z);
